...
def default_transform(img_size: int):
    """Apply the same preprocessing as training/validation to a single PIL
    image.

    This mirrors the Resize->CenterCrop->ToTensor->Normalize pipeline used in
    by resnet (our model).
    """
    ...
    tfm = transforms.Compose(
        [
            ...
            # Converts a PIL image (H×W×C, 0..255) to a float32 tensor C×H×W
            # scaled to [0, 1].
            transforms.ToTensor(),
            # Puts inputs on the same scale the pretrained weights were trained
            # on.
